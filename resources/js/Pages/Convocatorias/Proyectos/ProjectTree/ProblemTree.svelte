<script>
    ...
    let canCreateIDi =
        authUser.can.find((element) => element == 'idi.create') == 'idi.create'
    let canEditIDi =
        authUser.can.find((element) => element == 'idi.edit') == 'idi.edit'
    ...
    function showEfectoindirectoDialog(efectoIndirecto, efectoDirectoId) {
        reset()
        code =
            efectoIndirecto?.id != null
                ? 'EFE-' +
                  efectoIndirecto.efecto_directo_id +
                  '-IND-' +
                  efectoIndirecto.id
                : ''
        ...
        if (efectoIndirecto != null) {
            ...
            $formEfectoIndirecto.efecto_directo_id =
                efectoIndirecto.efecto_directo_id
        } else {
    ...
    function showStatementProblemDialog() {
        ...
        $formPlanteamientoProblema.justificacion_problema =
            proyecto.justificacion_problema
    }

    function submitPlanteamientoProblema() {
        if (canCreateOrUpdate == undefined) {
            $formPlanteamientoProblema.post(
                route('proyectos.planteamiento-problema', proyecto.id),
                {
                    onStart: () => {
                        sending = true
                    },
    ...
    function showCausaIndirectaDialog(causaIndirecta, causaDirectaId) {
        reset()
        code =
            causaIndirecta?.id != null
                ? 'CAU-' +
                  causaIndirecta.causa_directa_id +
                  '-IND-' +
                  causaIndirecta.id
                : ''
        ...
        if (causaIndirecta != null) {
            ...
            $formCausaIndirecta.causa_directa_id =
                causaIndirecta.causa_directa_id
        } else {
    ...
    onMount(() => {
        const efectoIndirecto = document.querySelector(
            '#efecto-indirecto-tooltip-placement',
        )
        const efectoIndirectoTooltip = document.querySelector(
            '#efecto-indirecto-tooltip',
        )
        const arrowEfectoIndirecto = document.querySelector(
            '#arrow-efecto-indirecto',
        )

        const efectoDirecto = document.querySelector(
            '#efecto-directo-tooltip-placement',
        )
        const efectoDirectoTooltip = document.querySelector(
            '#efecto-directo-tooltip',
        )
        const arrowEfectoDirecto = document.querySelector(
            '#arrow-efecto-directo',
        )

        const planteamientoProblema = document.querySelector(
            '#planteamiento-problema-tooltip-placement',
        )
        const planteamientoProblemaTooltip = document.querySelector(
            '#planteamiento-problema-tooltip',
        )
        const arrowPlanteamientoProblema = document.querySelector(
            '#arrow-planteamiento-problema',
        )

        const causaDirecta = document.querySelector(
            '#causa-directa-tooltip-placement',
        )
        const causaDirectaTooltip = document.querySelector(
            '#causa-directa-tooltip',
        )
        const arrowDirectCause = document.querySelector('#arrow-causa-directa')

        const causaIndirecta = document.querySelector(
            '#causa-indirecta-tooltip-placement',
        )
        const causaIndirectaTooltip = document.querySelector(
            '#causa-indirecta-tooltip',
        )
        const arrowCausaIndirecta = document.querySelector(
            '#arrow-causa-indirecta',
        )
...
<AuthenticatedLayout>
    ...
    <div class="py-12">
        ...
        <div class="mt-16">
            ...
            <div class="flex mb-14">
                {#each efectosDirectos as efectoDirecto, i}
                    <div class="flex-1">
                        ...
                        >
                            {#each efectoDirecto.efectos_indirectos as efectoIndirecto}
                                <div class="flex-1 efectos-directos relative">
                                    <div
                                        on:click={showEfectoindirectoDialog(
                                            efectoIndirecto,
                                            efectoDirecto.id,
                                        )}
                                        class="{efectoIndirecto.descripcion !=
                                        null
                                    ...
                                    >
                                        ...
                                        >
                                            <small
                                                ...
                                                >EFE-{efectoDirecto.id}-IND-{efectoIndirecto.id}</small
                                            >
                            ...
                            {#each { length: 3 - efectoDirecto.efectos_indirectos.length } as _empty}
                                <div
                                    class="flex-1 efectos-directos relative"
                                    on:click={showEfectoindirectoDialog(
                                        null,
                                        efectoDirecto.id,
                                    )}
                        ...
                        <!-- Efecto directo -->
                        <div
                            class="efectos-directos relative flex-1"
                            id={i == 0
                                ? 'efecto-directo-tooltip-placement'
                                : ''}
                            aria-describedby={i == 0 ? 'tooltip' : ''}
                        >
                            <div
                                on:click={showEfectoDirectoDialog(
                                    efectoDirecto,
                                )}
                                class="{efectoDirecto.descripcion != null
                                    ? 'bg-indigo-500 hover:bg-indigo-600'
                                    : 'bg-indigo-300 hover:bg-indigo-400'} h-36 rounded shadow-lg cursor-pointer mr-1.5"
                            >
                                <p
            ...
            <!-- Planteamiento del problema -->
            <div
                id="planteamiento-problema-tooltip"
                class="tooltip bg-black"
                role="tooltip"
                data-popper-placement="left"
            >
                ...
                >
                <div
                    id="arrow-planteamiento-problema"
                    class="arrow"
                    data-popper-arrow
                />
            </div>
            <div
                class="planteamiento-problema relative"
                id="planteamiento-problema-tooltip-placement"
                aria-describedby="tooltip"
            >
                <div
                    on:click={showStatementProblemDialog}
                    class="h-36 {proyecto.planteamiento_problema != null
                        ? 'bg-indigo-500 hover:bg-indigo-600'
            ...
            <div class="flex mt-14">
                {#each causasDirectas as causaDirecta, i}
                    <div class="flex-1">
                        ...
                        {/if}
                        <div
                            class="causas-directas relative flex-1"
                            id={i == 0 ? 'causa-directa-tooltip-placement' : ''}
                            aria-describedby={i == 0 ? 'tooltip' : ''}
                        >
                            <div
                                on:click={showCausaDirectaDialog(causaDirecta)}
                                class="{causaDirecta.descripcion != null
                                    ? 'bg-indigo-500 hover:bg-indigo-600'
                                    : 'bg-indigo-300 hover:bg-indigo-400'} h-36 rounded shadow-lg cursor-pointer mr-1.5"
                            >
                                <p
                        ...
                        >
                            {#each causaDirecta.causas_indirectas as causaIndirecta}
                                <div class="causas-directas relative flex-1">
                                    <div
                                        on:click={showCausaIndirectaDialog(
                                            causaIndirecta,
                                            causaDirecta.id,
                                        )}
                                        class="{causaIndirecta.descripcion !=
                                        null
                                            ? 'bg-indigo-500 hover:bg-indigo-600'
                                            : 'bg-indigo-300 hover:bg-indigo-400'} h-36 rounded shadow-lg cursor-pointer mr-1.5"
                                    >
                                        ...
                                        >
                                            <small
                                                class="title block font-bold mb-2"
                                                >CAU-{causaDirecta.id}-IND-{causaIndirecta.id}</small
                                            >
    ...
    <Dialog bind:open={dialog_open}>
        ...
        <div slot="content">
            {#if showCausaIndirectaForm}
                <form
                    on:submit|preventDefault={submitCausaIndirecta}
                    id="causa-indirecta"
                >
                    <fieldset disabled={canCreateOrUpdate}>
                        <div class="mt-4">
                            ...
                            <Textarea
                                ...
                                bind:value={$formCausaIndirecta.descripcion}
                                required
                            />
                        </div>
                    </fieldset>
            ...
            {:else if showCausaDirectaForm}
                <form
                    on:submit|preventDefault={submitCausaDirecta}
                    id="causa-directa"
                >
                    <fieldset disabled={canCreateOrUpdate}>
                        <div class="mt-4">
                            ...
                            <Textarea
                                ...
                                id="descripcion"
                                error={errors.descripcion}
                                bind:value={$formCausaDirecta.descripcion}
                                required
                            />
                        </div>
                    </fieldset>
            ...
            {:else if showEfectoIndirectoForm}
                <form
                    on:submit|preventDefault={submitEfectoIndirecto}
                    id="efecto-indirecto"
                >
                    <fieldset disabled={canCreateOrUpdate}>
                        <div class="mt-4">
                            ...
                            <Textarea
                                ...
                                id="descripcion"
                                error={errors.descripcion}
                                bind:value={$formEfectoIndirecto.descripcion}
                                required
                            />
                        </div>
                    </fieldset>
            ...
            {:else if showPlanteamientoProblemaForm}
                <form
                    on:submit|preventDefault={submitPlanteamientoProblema}
                    id="planteamiento-problema"
                >
                    <fieldset disabled={canCreateOrUpdate}>
                        <div class="mt-4">
                            <Label
                                required
                                class="mb-4"
                                labelFor="planteamiento_problema"
                            ...
                            <Textarea
                                ...
                                error={errors.planteamiento_problema}
                                bind:value={$formPlanteamientoProblema.planteamiento_problema}
                                required
                            />
                        </div>
                        ...
                        <div class="mt-4">
                            <Label
                                required
                                class="mb-4"
                                labelFor="justificacion_problema"
                                value="Justificación"
                            />
                            <InfoMessage
                            ...
                            <Textarea
                                ...
                                error={errors.justificacion_problema}
                                bind:value={$formPlanteamientoProblema.justificacion_problema}
                                required
                            />
                        </div>
                    </fieldset>
            ...
            {:else if showEfectoDirectoForm}
                <form
                    on:submit|preventDefault={submitEfectoDirecto}
                    id="efecto-directo"
                >
                    <fieldset disabled={canCreateOrUpdate}>
                        <div class="mt-4">
                            ...
                            <Textarea
                                ...
                                id="descripcion"
                                error={errors.descripcion}
                                bind:value={$formEfectoDirecto.descripcion}
                                required
                            />
                        </div>
                    </fieldset>

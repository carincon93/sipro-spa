    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset class="p-8" disabled={isSuperAdmin || checkPermission(authUser, [3, 4, 6, 7, 9, 10]) ? undefined : true}>
                ...
                <div class="mt-4">
                    <Input label="Número de meses que requiere el apoyo" id="numero_meses" type="number" input$min="1" input$step="0.5" input$max={proyecto.diff_meses == 10 ? 11.5 : proyecto.diff_meses} class="mt-1" error={errors.numero_meses} bind:value={$form.numero_meses} required />
                </div>

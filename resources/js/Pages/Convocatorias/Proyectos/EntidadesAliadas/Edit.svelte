<script>
    ...
    let form = useForm({
        ...
        descripcion_convenio: entidadAliada.entidad_aliada_idi ? entidadAliada.entidad_aliada_idi?.descripcion_convenio : '',
        grupo_investigacion: entidadAliada.entidad_aliada_idi ? entidadAliada.entidad_aliada_idi?.grupo_investigacion : '',
        ...
        <div class="bg-white rounded shadow max-w-3xl flex-1">
            <form on:submit|preventDefault={submit}>
                ...
                <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center sticky bottom-0">
                    ...
                    {/if}
                    {#if isSuperAdmin || (checkPermission(authUser, [3, 4, 9, 10]) && proyecto.modificable == true)}
                        <LoadingButton loading={$form.processing} class="ml-auto" type="submit">Editar entidad aliada</LoadingButton>
                    {/if}
                </div>
            </form>
        </div>

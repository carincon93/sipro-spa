...
<form on:submit|preventDefault={submit} class="mt-4 p-4">
    {#if proyectoAnexoId}
        <div class="bg-green-200 px-2 py-1 text-green-700">
            <span class="inline-block mb-4">Archivo cargado (<strong>Fecha de carga de archivo: </strong>{fechaActualizacion}). Por favor descargue el archivo desde el siguiente enlace:</span>
            <a target="_blank" class="text-indigo-400 underline inline-block mb-4 font-black flex" download href={route('convocatorias.proyectos.proyecto-anexos.download', [convocatoria.id, proyecto.id, proyectoAnexoId])}>

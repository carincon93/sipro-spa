...
<AuthenticatedLayout>
    ...
    <Dialog bind:open={proyectoDialogOpen} id="informacion">
                ...
                <ul>
                    {#each versiones as version, i}
                        <li class="bg-white p-2 rounded text-black text-xs">
                            {#if version.estado == 1}
                                <a href={route('convocatorias.proyectos.version', [convocatoria.id, tp.proyecto.id, version.version])}>Descargar versión {i}</a>
                            {:else}
        ...
        <div slot="content">
            <div>
                {#if JSON.parse(tp.proyecto.estado)?.requiereSubsanar == true && tp.proyecto.mostrar_recomendaciones == true && tp.proyecto.mostrar_requiere_subsanacion == true}
                ...
                {:else if JSON.parse(tp.proyecto.estado)?.requiereSubsanar == false && tp.proyecto.mostrar_recomendaciones == true && tp.proyecto.mostrar_requiere_subsanacion == true}
                    <div>

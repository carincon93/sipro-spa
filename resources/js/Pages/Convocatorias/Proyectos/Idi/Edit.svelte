...
<AuthenticatedLayout>
    ...
    <form on:submit|preventDefault={submit}>
        <fieldset class="p-8 divide-y" disabled={idi.proyecto.allowed.to_update ? undefined : true}>
                        ...
                        {#each idi.proyecto.evaluaciones as evaluacion, i}
                            {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                                <div class="bg-zinc-900 p-4 rounded shadow text-white my-2">
                                    ...
                                    <p class="whitespace-pre-line text-xs">{evaluacion.idi_evaluacion.titulo_comentario ? evaluacion.idi_evaluacion.titulo_comentario : 'Sin recomendación'}</p>
                                </div>
                            {/if}
                        {/each}
                    ...
                    <RecomendacionEvaluador class="mt-8">
                        {#each idi.proyecto.evaluaciones as evaluacion, i}
                            {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                                <div class="bg-zinc-900 p-4 rounded shadow text-white my-2">
                                    <p class="text-xs">Evaluador COD-{evaluacion.id}:</p>
                                    <p class="whitespace-pre-line text-xs">{evaluacion.idi_evaluacion.fechas_comentario ? evaluacion.idi_evaluacion.fechas_comentario : 'Sin recomendación'}</p>
                                </div>
                            {/if}
                        {/each}
                    </RecomendacionEvaluador>
            ...
            <fieldset class="py-24" disabled>
                ...
                {#if $form.centro_formacion_id}
                    <div class="mt-44 grid grid-cols-2">
                        ...
                        <div>
                            <Select id="linea_investigacion_id" items={lineasInvestigacion} bind:selectedValue={$form.linea_investigacion_id} error={errors.linea_investigacion_id} autocomplete="off" placeholder="Busque por el nombre de la línea de investigación, centro de formación, grupo de investigación o regional" required />
                        </div>
                    </div>
                {/if}
                <div class="mt-44 grid grid-cols-2">
            ...
            <div class="py-24">
                <p class="text-center mb-8">¿Cuál es el origen de las muestras con las que se realizarán las actividades de investigación, bioprospección y/o aprovechamiento comercial o industrial?</p>
                    ...
                    {#if isSuperAdmin || idi.proyecto.allowed.to_update}
                        <div class="bg-violet-100 p-5 mt-10">
                            ...
                            <div class="grid grid-cols-2">
                                ...
                                <div>
                                    <Select items={tecnoacademias} id="tecnoacademia_id" bind:selectedValue={$form.tecnoacademia_id} selectFunctions={[(event) => selectLineasTecnoacademia(event)]} error={errors.tecnoacademia_id} autocomplete="off" placeholder="Seleccione una opción" required />
            ...
            <div class="py-24">
                <div class="grid grid-cols-2">
                    ...
                    <div>
                        <SelectMulti id="programas_formacion" bind:selectedValue={$form.programas_formacion} items={programasFormacionConRegistroCalificado} isMulti={true} error={errors.programas_formacion} placeholder="Buscar por el nombre del programa de formación" required />
                    </div>
                </div>
            </div>
                        ...
                        <RecomendacionEvaluador class="mt-8">
                            {#each idi.proyecto.evaluaciones as evaluacion, i}
                                {#if isSuperAdmin || (evaluacion.finalizado && evaluacion.habilitado)}
                                    <div class="bg-zinc-900 p-4 rounded shadow text-white my-2">
                                        <p class="text-xs">Evaluador COD-{evaluacion.id}:</p>
                                        <p class="whitespace-pre-line text-xs">{evaluacion.idi_evaluacion.bibliografia_comentario ? evaluacion.idi_evaluacion.bibliografia_comentario : 'Sin recomendación'}</p>
                                    </div>
                                {/if}
                            {/each}
                        </RecomendacionEvaluador>

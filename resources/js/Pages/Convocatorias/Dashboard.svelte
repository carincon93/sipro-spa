...
<AuthenticatedLayout>
    <div class="py-12">
        {#if canShowConvocatorias || canEditConvocatorias || canDestroyConvocatorias || isSuperAdmin}
            <div class="flex justify-center items-center flex-col">
                <p>
                    Si desea revisar, {#if canEditConvocatorias} editar {/if} la
                    información de la convocatoria, de clic en el siguiente botón
                </p>
                <div>
                    <Button
                        on:click={() =>
                            Inertia.visit(
                                route('convocatorias.edit', [convocatoria.id]),
                            )}
                        class="mt-8 mb-20"
                        variant="raised"
                    >
        ...
        <h1 class="text-4xl text-center">
            ...
            Formule proyectos de I+D+i, Tecnoacademia-Tecnoparque y de Servicios
            Tecnológicos para la vigencia {convocatoria.year}
        </h1>
        <div class="grid grid-cols-3 gap-10 mt-24">
            {#if canIndexIDi || isSuperAdmin}
                <a
                    use:inertia
                    href={route('convocatorias.idi.index', convocatoria.id)}

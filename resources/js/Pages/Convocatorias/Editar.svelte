<script>
    ...
    let canIndexCallSenovaRoles =
        authUser.can.find(
            (element) => element == 'convocatoria-sennova-roles.index',
        ) == 'convocatoria-sennova-roles.index'
    let canShowCallSenovaRoles =
        authUser.can.find(
            (element) => element == 'convocatoria-sennova-roles.show',
        ) == 'convocatoria-sennova-roles.show'
    let canCreateCallSenovaRoles =
        authUser.can.find(
            (element) => element == 'convocatoria-sennova-roles.create',
        ) == 'convocatoria-sennova-roles.create'
    let canEditCallSenovaRoles =
        authUser.can.find(
            (element) => element == 'convocatoria-sennova-roles.edit',
        ) == 'convocatoria-sennova-roles.edit'
    let canDestroyCallSenovaRoles =
        authUser.can.find(
            (element) => element == 'convocatoria-sennova-roles.delete',
        ) == 'convocatoria-sennova-roles.delete'
    ...
    let form = useForm({
        ...
        max_fecha_finalizacion_proyectos:
            convocatoria.max_fecha_finalizacion_proyectos,
    })
    ...
    function destroy() {
        if (canDestroyConvocatorias || isSuperAdmin) {
            $deleteForm.delete(
                route('convocatorias.destroy', [convocatoria.id]),
                {
                    preserveScroll: true,
                },
            )
...
<AuthenticatedLayout>
    ...
    {#if canIndexCallSenovaRoles || canShowCallSenovaRoles || canCreateCallSenovaRoles || canEditCallSenovaRoles || canDestroyCallSenovaRoles || isSuperAdmin}
        <Button
            ...
            on:click={() =>
                Inertia.visit(
                    route(
                        'convocatorias.convocatoria-sennova-roles.index',
                        convocatoria.id,
                    ),
                )}
            class="mb-4"
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditConvocatorias || isSuperAdmin
                    ? undefined
                    : true}
            >
                ...
                <div class="mt-4">
                    ...
                    <div class="mt-4 flex items-start justify-around">
                        ...
                        <div
                            class="mt-4 flex {errors.max_fecha_finalizacio_proyectos
                                ? ''
                                : 'items-center'}"
                        >
                            <Label
                                ...
                                class={errors.max_fecha_finalizacio_proyectos
                                    ? 'top-3.5 relative'
                                    : ''}
                                value="hasta"
                            />
    ...
    <Dialog bind:open={dialog_open}>
        ...
        <div slot="content">
            <p>
                ...
                <br />
                Una vez eliminada la convocatoria, todos los proyectos y datos asociados

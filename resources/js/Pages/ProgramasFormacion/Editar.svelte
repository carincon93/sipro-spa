<script>
    ...
    let form = useForm({
        ...
        modalidad: {
            ...
            label: modalidades.find(
                (item) => item.value == programaFormacion.modalidad,
            )?.label,
        },
    ...
    function submit() {
        if (canEditProgramasFormacion || isSuperAdmin) {
            $form.put(
                route('programas-formacion.update', programaFormacion.id),
                {
    ...
    function destroy() {
        if (canDestroyProgramasFormacion || isSuperAdmin) {
            $form.delete(
                route('programas-formacion.destroy', programaFormacion.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditProgramasFormacion || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">

<script>
    ...
    export let reglaRolSt

    $: $title = 'Editar regla de rol ST'
    ...
    function submit() {
        if (isSuperAdmin) {
            $form.put(route('reglas-roles-st.update', reglaRolSt.id), {
                preserveScroll: true,
            })
        }
    }
    ...
    function destroy() {
        if (isSuperAdmin) {
            $form.delete(route('reglas-roles-st.destroy', reglaRolSt.id))
        }
    }
</script>
    ...
    <header class="shadow bg-white" slot="header">
        <div class="flex items-center justify-between lg:px-8 max-w-7xl mx-auto px-4 py-6 sm:px-6">
            <div>
                <h1>
                    {#if isSuperAdmin}
                        <a use:inertia href={route('reglas-roles-st.index')} class="text-cyan-400 hover:text-cyan-600"> Reglas de roles ST </a>
                    {/if}
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            ...
            <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center sticky bottom-0">
                {#if isSuperAdmin}
                    <button class="text-red-600 hover:underline text-left" tabindex="-1" type="button" on:click={() => (dialogOpen = true)}> Eliminar regla </button>
                {/if}
                {#if isSuperAdmin}
                    <LoadingButton loading={$form.processing} class="ml-auto" type="submit">Editar regla</LoadingButton>
                {/if}
            </div>
        </form>
    </div>

<script>
    ...
    function destroy() {
        if (isSuperAdmin) {
            $form.delete(route('segundo-grupo-presupuestal.destroy', rubroSegundoGrupoPresupuestal.id))
        }
    }
</script>
        ...
        <div class="flex items-center justify-between lg:px-8 max-w-7xl mx-auto px-4 py-6 sm:px-6">
            <div>
                <h1 class="overflow-ellipsis overflow-hidden w-breadcrumb-ellipsis whitespace-nowrap">
                    {#if isSuperAdmin}
                        <a use:inertia href={route('segundo-grupo-presupuestal.index')} class="text-indigo-400 hover:text-indigo-600"> Segundo grupo presupuestal </a>
                    {/if}

<script>
    ...
    function submit() {
        if (canEditTematicasEstrategicas || isSuperAdmin) {
            $form.put(
                route('tematicas-estrategicas.update', tematicaEstrategica.id),
                {
    ...
    function destroy() {
        if (canDestroyTematicasEstrategicas || isSuperAdmin) {
            $form.delete(
                route('tematicas-estrategicas.destroy', tematicaEstrategica.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditTematicasEstrategicas || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
            ...
            >
                {#if canDestroyTematicasEstrategicas || isSuperAdmin}
                    ...
                    >
                        Eliminar temática estratégica SENA
                    </button>
                {/if}
                {#if canEditTematicasEstrategicas || isSuperAdmin}
                    <LoadingButton
                        ...
                        type="submit"
                    >
                        Editar temática estratégica SENA
                    </LoadingButton>
                {/if}
            </div>
        </form>

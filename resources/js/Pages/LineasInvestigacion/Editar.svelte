<script>
    ...
    function submit() {
        if (canEditLineasInvestigacion || isSuperAdmin) {
            $form.put(
                route('lineas-investigacion.update', lineaInvestigacion.id),
                {
    ...
    function destroy() {
        if (canDestroyLineasInvestigacion || isSuperAdmin) {
            $form.delete(
                route('lineas-investigacion.destroy', lineaInvestigacion.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    <header class="shadow bg-white" slot="header">
        ...
        >
            <div>
                <h1>
                    {#if canIndexLineasInvestigacion || canShowLineasInvestigacion || canEditLineasInvestigacion || canDestroyLineasInvestigacion || isSuperAdmin}
                        <a
                            use:inertia
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditLineasInvestigacion || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">

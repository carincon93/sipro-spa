    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            ...
            <div class="shadow-inner bg-violet-200 border-violet-400 bottom-0 flex items-center justify-between mt-14 px-8 py-4 sticky">
                {#if isSuperAdmin}
                    <button class="text-red-600 hover:underline text-left" tabindex="-1" type="button" on:click={() => (dialogOpen = true)}> Eliminar línea de investigación </button>
                {/if}
                {#if isSuperAdmin || (checkRole(authUser, [4]) && authUser.centro_formacion_id == lineaInvestigacion.grupo_investigacion.centro_formacion_id) || checkRole(authUser, [21, 20, 18, 19, 5, 17])}
                    <LoadingButton loading={$form.processing} class="ml-auto" type="submit">Editar línea de investigación</LoadingButton>
                {/if}
            </div>
        </form>
    </div>

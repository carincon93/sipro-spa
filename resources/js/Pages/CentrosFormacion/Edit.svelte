<script>
    ...
    function submit() {
        if (canEditCentrosFormacion || isSuperAdmin) {
    ...
    function destroy() {
        if (canDestroyCentrosFormacion || isSuperAdmin) {
...
<AuthenticatedLayout>
    <header class="shadow bg-white" slot="header">
        <div class="flex items-center justify-between lg:px-8 max-w-7xl mx-auto px-4 py-6 sm:px-6">
            <div>
                <h1>
                    {#if canIndexCentrosFormacion || canShowCentrosFormacion || canEditCentrosFormacion || canDestroyCentrosFormacion || isSuperAdmin}
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset class="p-8" disabled={canEditCentrosFormacion || isSuperAdmin ? undefined : true}>
                <div class="mt-4">
            ...
            <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center sticky bottom-0">
                {#if canDestroyCentrosFormacion || isSuperAdmin}
                    <button class="text-red-600 hover:underline text-left" tabindex="-1" type="button" on:click={(event) => (dialog_open = true)}> Eliminar centro de formación </button>
                {/if}
                {#if canEditCentrosFormacion || isSuperAdmin}

<script>
    ...
    let form = useForm({
        ...
        categoria_minciencias: {
            ...
            label: categoriasMinciencias.find(
                (item) =>
                    item.value == grupoInvestigacion.categoria_minciencias,
            )?.label,
        },
    ...
    function submit() {
        if (canEditGruposInvestigacion || isSuperAdmin) {
            $form.put(
                route('grupos-investigacion.update', grupoInvestigacion.id),
                {
    ...
    function destroy() {
        if (canDestroyGruposInvestigacion || isSuperAdmin) {
            $form.delete(
                route('grupos-investigacion.destroy', grupoInvestigacion.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    <header class="shadow bg-white" slot="header">
        ...
        >
            <div>
                <h1>
                    {#if canShowGruposInvestigacion || canEditGruposInvestigacion || canDestroyGruposInvestigacion || isSuperAdmin}
                        <a
                            use:inertia
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditGruposInvestigacion || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
                ...
                <div class="mt-4">
                    ...
                    <Input
                        ...
                        type="url"
                        class="mt-1 block w-full"
                        bind:value={$form.enlace_gruplac}
                        error={errors.enlace_gruplac}
                        required
                    />
                </div>
                ...
                <div class="mt-4">
                    ...
                    <Input
                        ...
                        type="text"
                        class="mt-1 block w-full"
                        bind:value={$form.codigo_minciencias}
                        error={errors.codigo_minciencias}
                        required
                    />
                </div>
                ...
                <div class="mt-4">
                    <Label
                        required
                        class="mb-4"
                        labelFor="categoria_minciencias"
                        value="Categoría Minciencias"
                    />
                    <Select
                        id="categoria_minciencias"
                        items={categoriasMinciencias}
                        bind:selectedValue={$form.categoria_minciencias}
                        error={errors.categoria_minciencias}
                        autocomplete="off"

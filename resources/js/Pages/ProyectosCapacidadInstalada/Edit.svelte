<script>
    ...
    async function syncColumnLong(column, form) {
        return new Promise((resolve) => {
            if ((typeof column !== 'undefined' && typeof form !== 'undefined' && isSuperAdmin) || (typeof column !== 'undefined' && typeof form !== 'undefined' && checkRole(authUser, [4, 6]) && authUser.id == autorPrincipal.id)) {
                Inertia.put(
...
<AuthenticatedLayout>
    ...
    <form on:submit|preventDefault={submit}>
        <fieldset class="p-8" disabled={isSuperAdmin || (proyectoCapacidadInstalada.estado_proyecto != 'Finalizado' && checkRole(authUser, [4, 6]) && authUser.id == autorPrincipal.id) ? undefined : true}>
            <div class="mt-28">

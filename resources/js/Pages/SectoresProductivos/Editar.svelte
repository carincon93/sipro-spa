<script>
    ...
    function submit() {
        if (canEditSectoresProductivos || isSuperAdmin) {
            $form.put(
                route('sectores-productivos.update', sectorProductivo.id),
                {
    ...
    function destroy() {
        if (canDestroySectoresProductivos || isSuperAdmin) {
            $form.delete(
                route('sectores-productivos.destroy', sectorProductivo.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditSectoresProductivos || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
